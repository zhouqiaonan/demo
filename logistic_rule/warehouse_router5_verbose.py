...
def find_warehouse(order, rules_list):
    ...
    Prints a detailed reasoning trace for every rule checked.
    """
    print("\n" + "="*80)
    print("STARTING ALLOCATION LOGIC")
    print("="*80)

    # 1. Standardize and Validate Order Inputs
    try:
    ...
    order_type_name = RULE_TYPE_NAMES.get(order_type, str(order_type))
    order_prime_status = "Yes" if order_is_prime == 1 else "No"

    print(f"ORDER DETAILS:")
    print(f" - Type: {order_type_name} | Prime: {order_prime_status}")
    print(f" - Dest: {order_country} ({order_zip})")
    print(f" - Item: {order_sku} from {order_shop}")
    print("-" * 80)
    print(f"Analyzing {len(rules_list)} active rules sorted by Priority (Sort Order)...\n")
    ...
    for index, rule in enumerate(rules_list):
        ...
        # Log the start of checking a rule
        print(f"[{index+1}] Checking Rule ID {rule_id}: '{rule_title}' (Priority: {priority})")

        # --- GATE 1: Active ---
        # (Already filtered by compile_rules, but good for completeness)
        if not rule['active']:
            print(f"    >> [REJECTED] Rule is inactive.")
            continue 

        # --- GATE 2: Rule Type ---
        if rule['rule_type'] != order_type:
            r_type_name = RULE_TYPE_NAMES.get(rule['rule_type'], str(rule['rule_type']))
            print(f"    >> [REJECTED] Gate 2 (Type): Rule requires '{r_type_name}', Order is '{order_type_name}'.")
            continue 

        # --- GATE 3: Prime Status ---
        if rule['is_prime'] != order_is_prime:
            r_prime = "Yes" if rule['is_prime'] == 1 else "No"
            print(f"    >> [REJECTED] Gate 3 (Prime): Rule requires Prime={r_prime}, Order is Prime={order_prime_status}.")
            continue

        # --- GATE 4: Country ---
        if rule['allowed_countries']: 
            if order_country not in rule['allowed_countries']:
                print(f"    >> [REJECTED] Gate 4 (Country): Order country '{order_country}' not in allowed list {list(rule['allowed_countries'])}.")
                continue

        # --- GATE 5: Shop ---
        if rule['allowed_shops']:
            if order_shop not in rule['allowed_shops']:
                print(f"    >> [REJECTED] Gate 5 (Shop): Order shop '{order_shop}' not in allowed list {list(rule['allowed_shops'])}.")
                continue

        # --- GATE 6: SKU ---
        if rule['allowed_skus']:
            if order_sku not in rule['allowed_skus']:
                print(f"    >> [REJECTED] Gate 6 (SKU): Order SKU '{order_sku}' not in allowed list.")
                continue
        ...
        if rule['allowed_zip_ranges']:
            ...
            if not is_in_range:
                print(f"    >> [REJECTED] Gate 7 (Postcode): Zip {order_zip} is not within any allowed ranges for this rule.")
                continue 

        # === WINNER ===
        print(f"    >> [ACCEPTED] All Gates Passed!")
        print("="*80)
        return {
            'status': 'success',
    ...
    # 3. No Match Found
    print("\n" + "="*80)
    print("END OF LOGIC: No matching rule found.")
    print("="*80)
